...
namespace DeviceSimulationTool.Components
{
    ...
    /// </summary>
    public partial class WiegandTabItem : TabItem
    {
        #region Interface
        ...
        public static readonly DependencyProperty IsSelectedDeviceProperty =
            DependencyProperty.Register("IsSelectedDevice", typeof(bool), typeof(WiegandTabItem), new PropertyMetadata(false));
        ...
        public static readonly DependencyProperty IsStartProperty =
            DependencyProperty.Register("IsStart", typeof(bool), typeof(WiegandTabItem), new PropertyMetadata(false));
        ...
        public static readonly DependencyProperty IsDeviceFullProperty =
            DependencyProperty.Register("IsDeviceFull", typeof(bool), typeof(WiegandTabItem), new PropertyMetadata(false));
        ...
        public static readonly DependencyProperty StdoutProperty =
            DependencyProperty.Register("Stdout", typeof(string), typeof(WiegandTabItem), new PropertyMetadata(""));
        #endregion

        public static string PageName { get; } = "Wiegand";
        ...
        public WiegandTabItem()
        {
            ...
            catch (Exception ex)
            {
                ...
                App.PrintService.Log($"{WiegandTabItem.PageName}, {message}", Print.EMode.error);

                MessageBox.Show(message, WiegandTabItem.MessageBoxTitle, MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void StartService(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    App.PrintService.Log($"{WiegandTabItem.PageName}", Print.EMode.success);
                }
                catch (Exception ex)
                {
                    string message = Community.ConvertDataValidateExceptionToMessage(ex);

                    App.PrintService.Log($"{WiegandTabItem.PageName}, {message.Replace("\n", " ")}", Print.EMode.error);
            ...
            try
            {
                ...
                server.onMessage.Subscribe((x) =>
                {
                    this.Dispatcher.Invoke(new Action(() =>
                    {
                        ...
                        App.PrintService.Log($"{x}", Print.EMode.message, $"Wiegand_{device.item.DeviceSerialNumber}");
                ...
                App.PrintService.Log($"{device.config.name} is Start", Print.EMode.message, $"Wiegand_{device.item.DeviceSerialNumber}");
                if (this.SelectedDeviceSerialNumber == device.item.DeviceSerialNumber)
                {
                    if (this.Stdout != "")
                    {
        ...
        private void StopServer(IDevice device)
        {
            ...
            try
            {
                ...
                App.PrintService.Log($"{device.config.name} was Stop", Print.EMode.message, $"Wiegand_{device.item.DeviceSerialNumber}");
                device.stdouts.Add($"");
                App.LogService.Write($"", $"Wiegand_{device.item.DeviceSerialNumber}");
                if (this.SelectedDeviceSerialNumber == device.item.DeviceSerialNumber)
                {
                    if (this.Stdout != "")
                    {
